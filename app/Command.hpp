...
public:
    ...
    // ------------ MY OWN ADDED VARIABLES AND FUNCTIONS --------------

    // If the command executed with no errors,
    // set "didExecute" to true.
    // If the command could not execute and printed
    // an error message, set "didExecute" to false.
    //
    // Important for use in undoing and redoing actions.
    // Makes sure that program doesnt try undoing/redoing
    // a command inside the undo/redo stack that never executed.
    void setExecutionState(bool executed)
    {
    	didExecute = executed;
    }
